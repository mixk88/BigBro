...
import static com.example.BigBro.Const.*;

@Component
public class BigBroApplication extends TelegramLongPollingBot {
	...
	public void sendPost(String text){
		...
		log.info("run method sendPost");

		try{
			execute(sendMessage);
			log.info("execute send massage (sendpost)");
	...
	public void sendAnswer(Message message, String text){
		...
		sendMessage.setReplyToMessageId(message.getMessageId());
		sendMessage.setText(text);
		log.info("run method sendAnswer");

		try{
			execute(sendMessage);
			log.info("execute send massage (sendanswer)");
	...
	public void sendServMsg(String text){
		...
		sendMessage.setText(text);
		log.info("run sendServMsg method");
		try{
			execute(sendMessage);
		} catch (TelegramApiException e) {
			throw new RuntimeException(e);
	...
	public void onUpdateReceived(Update update) {
		log.info("OUR method running");
		Message message = update.getMessage();
		log.info("update message");
		boolean rightText;
		try {
			rightText = Censor.check(message.toString());
			log.info("check input text");
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	...
	public String getBotUsername() {
		log.info("run method getBotUsername");
		return BOTUSERNAME;
			}

	@Override
	public String getBotToken() {
		log.info("rum method getBotToken");
		return TOKEN;
	}
}
